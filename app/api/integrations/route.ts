...
async function sendResendTestEmail(to: string) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from: `FlowPilot <${from}>`, to: [to], subject: "FlowPilot Email Integration Test", html: "<h2>FlowPilot Email is working</h2><p>This test confirms that FlowPilot can send email through Resend.</p>" }),
  });
  const data = await response.json().catch(() => null);
  if (!response.ok) return { ok: false, message: data?.message || "Resend rejected the test email.", status: response.status };
...
export async function POST(request: Request) {
  ...
  if (body.provider === "webhook" && body.action === "create-webhook") {
    ...
    const integration = existing
      ? await db.integration.update({ where: { id: existing.id }, data: { enabled: true, accessToken: encrypt(secret), webhookUrl: "pending" } })
      : await db.integration.create({ data: { userId: user.id, organizationId, provider: "webhook", enabled: true, accessToken: encrypt(secret), webhookUrl: "pending" } });
    const endpoint = new URL(`/api/integrations/webhook/${integration.id}`, request.url).toString();
    await db.integration.update({ where: { id: integration.id }, data: { webhookUrl: endpoint } });
  ...
  if (body.provider === "email") {
    if (body.action === "send-test") {
      ...
      const integration = await db.integration.upsert({ where: { organizationId_provider: { organizationId, provider: "email" } }, create: { userId: user.id, organizationId, provider: "email", enabled: true, accessToken: "resend-env", accountEmail: user.email }, update: { enabled: true, accessToken: "resend-env", accountEmail: user.email } });
    ...
    const key = resendKey();
    if (!key || !key.startsWith("re_") || key.length < 20) return NextResponse.json({ error: "RESEND_API_KEY is missing or invalid in the deployed environment." }, { status: 400 });
  ...
  if (body.provider !== "zapier") return NextResponse.json({ error: "This integration must be connected through its dedicated setup flow" }, { status: 400 });
  if (!body.webhookUrl) return NextResponse.json({ error: "Zapier webhook URL is required" }, { status: 400 });
  const integration = await db.integration.upsert({ where: { organizationId_provider: { organizationId, provider: body.provider } }, create: { userId: user.id, organizationId, provider: body.provider, enabled: true, webhookUrl: body.webhookUrl }, update: { enabled: true, webhookUrl: body.webhookUrl } });
