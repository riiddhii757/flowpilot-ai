...
if(body.provider==="webhook"&&body.action==="create-webhook"){
  ...
  const integration=existing?await db.integration.update({where:{id:existing.id},data:{enabled:true,accessToken:encrypt(secret),webhookUrl:"pending"}}):await db.integration.create({data:{userId:user.id,organizationId,provider:"webhook",enabled:true,accessToken:encrypt(secret),webhookUrl:"pending"}});
